...
/** 0?€ ë¸”ëŸ¬ ?†ìŒ. `||` ë¡?0??ê¸°ë³¸ê°’ìœ¼ë¡?ë°”ê¾¸ì§€ ?ŠëŠ”?? */
...
/** ë¸”ëŸ¬ ê°•ë„(px)ë¥?ëª¨ì?´í¬ ë¸”ë¡ ?¬ê¸°(px)ë¡?ë³€?? 1px = ?ë³¸(?½ì????†ìŒ). */
...
export function MosaicRegionLayer({
  ...
  /** ë¯¸ë””?´ê? ?¤ì œ ?œì‹œ?˜ëŠ” ?¬ê°??px). ?ëµ ??ë°•ìŠ¤ ?„ì²´. objectFit=cover/fill??ë§ì¶° ?˜ê²¨ì¤?ê²? */
...
type BlurRegionEditorProps = {
  ...
  onClose: () => void
  /** cover: ?´ë²¤??ê·¸ë˜??16:9 ?¬ë¡­). fit: SNSì²˜ëŸ¼ ?ë³¸ ë¹„ìœ¨ ? ì? */
  layout?: 'cover' | 'fit'
}
      ...
      <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-indigo-500/25 bg-slate-900 shadow-2xl">
        <div className="flex items-start justify-between gap-3 border-b border-white/10 px-5 py-4">
          <div>
            ...
            <h3 className="mt-1 text-base font-semibold text-slate-100">ëª¨ì?´í¬ ?ì—­ ?¸ì§‘</h3>
            <p className="mt-1 text-xs text-slate-500">
              ?ë³¸ ?Œì¼?€ ?˜ì •?˜ì? ?ŠìŠµ?ˆë‹¤. ?¤ëª¨ë¥?ê·¸ë ¤ ê°€ë¦??„ì¹˜ë¥??•í•©?ˆë‹¤.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-auto p-5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-[11px] text-slate-500">
              {mode === 'draw' ? '?œë˜ê·¸í•´???¤ëª¨ë¥?ê·¸ë¦½?ˆë‹¤.' : '?¤ëª¨ë¥???¸°ê±°ë‚˜ ëª¨ì„œë¦¬ë? ?¡ì•„ ?¬ê¸°ë¥?ë°”ê¿‰?ˆë‹¤.'}
            </span>
          </div>
...
src={asset?.url}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <p className="text-[11px] font-semibold text-slate-400">???¤ëª¨ ê¸°ë³¸ ê°•ë„</p>
              <input
                type="range"
                min={BLUR_MIN}
                max={BLUR_MAX}
            ...
            <div className="space-y-2">
              <p className="text-[11px] font-semibold text-slate-400">? íƒ???ì—­ ê°•ë„</p>
              {selected ? (
                <>
                  <input
                    type="range"
                ...
                <p className="text-[11px] text-slate-500">? íƒ ëª¨ë“œ?ì„œ ?¤ëª¨ë¥?ê³ ë¥´ë©?ê°•ë„ë¥?ë°”ê? ???ˆìŠµ?ˆë‹¤.</p>
              )}
            </div>
          </div>
        </div>
