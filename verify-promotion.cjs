...
// 1) 실제 설정 확인
check(config.promotions.sme.requiredViewers === 5000, `sme 필요 시청자 = 5000 (실제: ${config.promotions.sme.requiredViewers})`)
...
check(r5000.promoted === true && r5000.grade === 'sme', `tiny+5000명 → sme 승급 (promoted=${r5000.promoted})`)

// 3) 승급 판정 — tiny, 4999명 → 실패
const r4999 = applyStationReview(config, 'tiny', 4999)
check(r4999.promoted === false, 'tiny+4999명 → 승급 실패(정상)')
...
check(blackReview.grade === 'tiny', `black+5000명 → tiny 승급 (grade=${blackReview.grade})`)
